						...
						<tbody>
							<c:forEach items="${myorders}" var="orders">
								<tr>
									...
									<td>${orders.goods_getprice}</td>
									<td><c:if test="${orders.order_state=='0'}">未付款</c:if>
									<c:if test="${orders.order_state=='1'}">未收货</c:if>
									<c:if test="${orders.order_state=='2'}">交易成功</c:if>
									<c:if test="${orders.order_state=='3'}">交易关闭</c:if>
									</td>
